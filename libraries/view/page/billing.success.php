<?php
$application = array("paypal"=>"PayPal","cc"=>"Credit Card","vm"=>"Create VM");
if(isset($application[$_GET["application"]])){
  $title = $application[$_GET["application"]];
  echo "<h2><i class='remove circle icon'></i>Complete Order</h2><hr style='margin-top:0px;margin-bottom:10px;' />";
  if(isset($_GET["token"]) and preg_match("/^[a-zA-Z0-9\-]*$/",$_GET["token"])==TRUE){$token="(".htmlspecialchars($_GET["token"]).")";} else{$token=null;}
  echo "".$er->return_success("Your process has been complete! <b>{$token}</b>")."";
...
} else{echo $er->fatal("0x002","Not found Result Page!");}
?>
